#!/bin/bash
# RRR Completion Notification Hook
...
case "$(uname -s)" in
  Darwin)
    osascript -e "display alert \"RRR: $project\" message \"$message\" as informational" &>/dev/null &
    ;;
  Linux)
    if command -v notify-send &>/dev/null; then
      notify-send "RRR: $project" "$message" --urgency=normal
    elif command -v zenity &>/dev/null; then
      zenity --info --title="RRR: $project" --text="$message" &
    fi
    ;;
  MINGW*|CYGWIN*|MSYS*)
    # Windows via PowerShell
    if command -v powershell.exe &>/dev/null; then
      powershell.exe -Command "Add-Type -AssemblyName System.Windows.Forms; [System.Windows.Forms.MessageBox]::Show('$message', 'RRR: $project', 'OK', 'Information')" &>/dev/null &
    fi
    ;;
esac

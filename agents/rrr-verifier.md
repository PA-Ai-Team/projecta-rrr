...
When gaps are found, structure them for consumption by `/rrr:plan-phase --gaps`.
...
The planner (`/rrr:plan-phase --gaps`) reads this gap analysis and creates appropriate plans.
...
Structured gaps in VERIFICATION.md frontmatter for `/rrr:plan-phase --gaps`.
...
**Structure gaps in YAML frontmatter.** The planner (`/rrr:plan-phase --gaps`) creates plans from your analysis.

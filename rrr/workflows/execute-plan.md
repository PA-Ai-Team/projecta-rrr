...
2. Use Task tool with subagent_type="rrr-executor":
...
2. For each segment in order:
   ...
   B. If routing = Subagent:
      ```
      Spawn Task tool with subagent_type="rrr-executor":
...
See `~/.claude/rrr/references/tdd.md` for TDD plan structure.
</tdd_plan_execution>
...
See ~/.claude/rrr/references/checkpoints.md for complete checkpoint guidance.
</step>
...
If you're reading this workflow because an orchestrator spawned you (vs running directly from /rrr:execute-plan), the orchestrator's prompt will include checkpoint return instructions. Follow those instructions when you hit a checkpoint.
...
Create `.planning/phases/XX-name/{phase}-USER-SETUP.md` using template from `~/.claude/rrr/templates/user-setup.md`.
...
For commit message conventions, see ~/.claude/rrr/references/git-integration.md
</step>
...
`/rrr:execute-plan .planning/phases/{phase-dir}/{phase}-{next-plan}-PLAN.md`
...
**Also available:**
- `/rrr:verify-work {phase}-{plan}` — manual acceptance testing before continuing
...
**Also available:**
- `/rrr:verify-work` — manual acceptance testing before completing milestone
- `/rrr:add-phase <description>` — add another phase before completing

...
**No dependency analysis needed.** Wave numbers are pre-computed during `/rrr:plan-phase`.
...
| `gaps_found` | Present gap summary, offer `/rrr:plan-phase {phase} --gaps` |
...
- `/rrr:verify-work {X}` — manual testing before planning
```

User runs `/rrr:plan-phase {X} --gaps` which:
1. Reads VERIFICATION.md gaps
2. Creates additional plans (04, 05, etc.) to close gaps
3. User then runs `/rrr:execute-phase {X}` again
...
1. Run `/rrr:execute-phase {phase}` again

...
- `/rrr:plan-phase {next}` — Plan next phase
- `/rrr:execute-phase {next}` — Execute next phase
```
</step>
...
- Follow @~/.claude/rrr/workflows/diagnose-issues.md
...
Next steps:
- `/rrr:plan-phase {phase} --gaps` — Create fix plans from diagnosed gaps
- `/rrr:verify-work {phase}` — Re-test after fixes
```
</step>

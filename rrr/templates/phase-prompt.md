...
> **Note:** Planning methodology is in `agents/rrr-planner.md`.
...
@~/.claude/rrr/references/checkpoints.md
</execution_context>
...
**Wave is pre-computed:** Wave numbers are assigned during `/rrr:plan-phase`. Execute-phase reads `wave` directly from frontmatter and groups plans by wave number. No runtime dependency analysis needed.
...
See `~/.claude/rrr/templates/user-setup.md` for full schema and examples
...
See `~/.claude/rrr/references/goal-backward.md` for derivation process.
See `~/.claude/rrr/workflows/verify-phase.md` for verification logic.

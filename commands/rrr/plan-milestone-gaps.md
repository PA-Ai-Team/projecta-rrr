...
<objective>
Create all phases necessary to close gaps identified by `/rrr:audit-milestone`.

Reads MILESTONE-AUDIT.md, groups gaps into logical phases, creates phase entries in ROADMAP.md, and offers to plan each phase.

One command creates all fix phases — no manual `/rrr:add-phase` per gap.
</objective>
...
```
No audit gaps found. Run `/rrr:audit-milestone` first.
```
...
**Also available:**
- `/rrr:execute-phase {N}` — if plans already exist
...
`/rrr:audit-milestone` — re-audit to verify gaps closed
`/rrr:complete-milestone {version}` — archive when audit passes
```
...
- [ ] Changes committed
- [ ] User knows to run `/rrr:plan-phase` next
</success_criteria>

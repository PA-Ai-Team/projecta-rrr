...
<objective>
Execute a single PLAN.md file by spawning the `rrr-executor` subagent.

Orchestrator stays lean: validate plan, spawn subagent, handle checkpoints, report completion. The `rrr-executor` has all execution logic baked in.
...
<checkpoint_handling>
When `rrr-executor` returns with checkpoint:
...
Spawn fresh `rrr-executor` with continuation context:
...
The `rrr-executor` has continuation handling baked in — it will verify previous commits and resume correctly.

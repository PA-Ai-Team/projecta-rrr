...
wave: 2
depends_on: ["16-01"]
files_modified: [commands/rrr/plan-phase.md]
autonomous: true
...
<objective>
Update /rrr:plan-phase orchestrator to spawn planner → checker → revise loop.

Purpose: Plans are validated before execution, catching issues early. User sees the ping-pong between agents.

Output: Updated commands/rrr/plan-phase.md with verification loop
</objective>
...
- Add `Task` to allowed-tools (needed for spawning checker)
  ...
  <verify>grep -E "^context:" commands/rrr/plan-phase.md (should return nothing)</verify>
...
- --skip-verify flag for experienced users
  ...
  <verify>
    - Process has 11 steps: `grep -c "^## [0-9]" commands/rrr/plan-phase.md`
    - Checker spawn exists: `grep -c "gsd-plan-checker" commands/rrr/plan-phase.md` (at least 2)
    - Revision loop exists: `grep -c "iteration" commands/rrr/plan-phase.md` (at least 3)
  </verify>
...
```
  ...
  <verify>grep -c "gsd-plan-checker" commands/rrr/plan-phase.md (at least 3 — in process and success_criteria)</verify>

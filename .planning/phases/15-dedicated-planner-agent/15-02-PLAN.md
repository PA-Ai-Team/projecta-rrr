...
wave: 2
depends_on: ["15-01"]
files_modified: [commands/rrr/plan-phase.md, rrr/workflows/plan-phase.md, rrr/templates/planner-subagent-prompt.md]
autonomous: true

must_haves:
  truths:
    - "/rrr:plan-phase command spawns gsd-planner agent"
    - "Orchestrator is thin (<200 lines)"
    - "Template provides context, agent has expertise"
  artifacts:
    - path: "commands/rrr/plan-phase.md"
      provides: "Thin orchestrator for phase planning"
      min_lines: 80
      contains: "agent: gsd-planner"
    - path: "rrr/templates/planner-subagent-prompt.md"
      provides: "Context-only prompt template for spawning planner"
      min_lines: 50
  key_links:
    - from: "commands/rrr/plan-phase.md"
...
<objective>
Refactor /rrr:plan-phase to thin orchestrator that spawns gsd-planner agent.
...
@commands/rrr/debug.md
@commands/rrr/research-phase.md
@rrr/templates/debug-subagent-prompt.md
@rrr/templates/research-subagent-prompt.md
...
<task type="auto">
  <name>Task 1: Refactor /rrr:plan-phase to thin orchestrator</name>
  <files>commands/rrr/plan-phase.md</files>
  <action>
Rewrite `commands/rrr/plan-phase.md` as thin orchestrator following debug.md and research-phase.md patterns.
...
- Scope estimation rules
  ...
  <verify>
File exists: `commands/rrr/plan-phase.md`
Has `agent: gsd-planner` in frontmatter
Line count under 200
No workflow or reference file inclusions in execution_context
  </verify>
  <done>/rrr:plan-phase refactored to thin orchestrator (~120-150 lines)</done>
</task>

<task type="auto">
  <name>Task 2: Deprecate workflows/plan-phase.md</name>
  <files>rrr/workflows/plan-phase.md</files>
  <action>
Replace `rrr/workflows/plan-phase.md` with deprecation notice following the pattern from workflows/debug.md and workflows/research-phase.md.
...
The `/rrr:plan-phase` command spawns the gsd-planner agent directly.
...
<task type="auto">
  ...
  <files>rrr/templates/planner-subagent-prompt.md</files>
  <action>
Create `rrr/templates/planner-subagent-prompt.md` following the pattern from debug-subagent-prompt.md and research-subagent-prompt.md.
...
**Key principle:** Template provides CONTEXT. Agent has EXPERTISE.
  ...
  <verify>
File exists: `rrr/templates/planner-subagent-prompt.md`
...
- [ ] commands/rrr/plan-phase.md is thin orchestrator (<200 lines)
...
<success_criteria>
- /rrr:plan-phase refactored to thin orchestrator
